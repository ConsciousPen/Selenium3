	...
	protected void setRawValue(List<TestData> testDataList) {
		for (TestData rowData : testDataList) {
			...
			for (String columnName : rowData.getKeys()) {
				if (getAssetCollection().get(columnName) instanceof StaticElement) {
			...
			Row fillableRow = searchRowByContains ? fillableTable.getRowContains(searchRowQuery) : fillableTable.getRow(searchRowQuery);
			fillableRow.verify.present(String.format("Can't find row in \"%1$s\" fillable table with %2$squery: %3$s", getName(), searchRowByContains ? "contains " : "", searchRowQuery.entrySet()));
