...
public class InquiryAssetList extends AssetList{
	...
	private void assetFieldMandatory(String message, String assetLabel, boolean isMandatory, String actualXpath) {
		if (isMandatory) {
			CustomAssert.assertTrue(message, actualXpath.contains("required"));
		} else {
			CustomAssert.assertFalse(message, actualXpath.contains("required"));
		}
	}
