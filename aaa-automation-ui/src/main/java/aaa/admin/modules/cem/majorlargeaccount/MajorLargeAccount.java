...
public class MajorLargeAccount implements IMajorLargeAccount {
    ...
    public String createWithId(TestData td) {
        initiate();
        getDefaultView().fill(td);
        String accountId = new CreateMajorLargeAccountTab().getAssetList().getControl(CemMetaData.CreateMajorLargeAccountTab.MAJOR_LARGE_ACCOUNT_ID.getLabel()).getValue().toString();
        CreateMajorLargeAccountTab.buttonSave.click();
        return accountId;
    }
