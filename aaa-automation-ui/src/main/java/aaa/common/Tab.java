...
public abstract class Tab {
    ...
    public static Button buttonNext = new Button(By.xpath("//input[@id='policyDataGatherForm:next_footer' or @id='crmForm:nextBtn_footer' or @id='policyDataGatherForm:next']"));
     ...
     * Note: normally the same TestData is passed here as in {@link products.Workspace.fill()}.
     */
    ...
    public Tab verifyFieldIsDisplayed(String label) {
        BaseElement<?, ?> control = assetList.getControl(label);
        CustomAssert.assertTrue("Field '" + label + "' must be displayed but it is not",
                control.isPresent() && control.isVisible());
        return this;
    }
    ...
    public Tab verifyFieldIsNotDisplayed(String label) {
        ...
        if (control.isPresent()) {
            CustomAssert.assertFalse("Field '" + label + "' must not be displayed but it is",
                    control.isVisible());
        }
        return this;
    }
    ...
    public Tab verifyFieldIsEnabled(String label) {
        CustomAssert.assertTrue("Field '" + label + "' must be enabled but it is not",
                assetList.getControl(label).isEnabled());
        return this;
    }

    public Tab verifyFieldIsDisabled(String label) {
        CustomAssert.assertFalse("Field '" + label + "' must be disabled but it is not",
                assetList.getControl(label).isEnabled());
        return this;
    }
    ...
    public Tab verifyFieldHasMessage(String label, String expectedValue) {
        ...
        String errorMessage = String.format("'%s' field's actual warning '%s' is not equal to the expected warning of '%s'",
                label, actualValue, expectedValue);
        CustomAssert.assertEquals(errorMessage, expectedValue, actualValue);
        return this;
    }
