 ...
 * 6. Verify that previously created Campaign is started
 * @details
 */
public class TestCustomerCampaignStart extends CemBaseTest {

    @Test
    @TestInfo(component = "CRM.Customer")
    public void testCustomerCampaignStart() {
        adminApp().open();
        ...
        log.info("TEST: Start Campaign # " + campaignId);
        campaign.startCampaign().perform(1);

        CampaignPage.tableCampaigns.getRow(1).getCell(CEMConstants.CEMCampaignsTable.STATUS).verify.value(CampaignEnum.CampaignStatus.ACTIVE.get());
    }
}
