 ...
 * 5. Perform Copy action and create Child Campaign
 * 6. Verify that previously created Child Campaign is present in Campaigns table
 * @details
 */
public class TestCustomerCampaignCreateChild extends CemBaseTest {

    @Test(groups = {"7.2_All_UM_CopyCampaign"})
    @TestInfo(component = "CRM.Customer")
    public void testCustomerCampaignCreateChild() {
        adminApp().open();
        ...
        log.info("TEST: Create Child Campaign # " + campaignId);
        campaign.createChildCampaign().perform(tdCampaign.getTestData("ChildCampaign", "TestData"), 1);

        String childCampaignId = CreateChildCampaignActionTab.labelCampaignId.getValue();

        campaign.navigate();
        campaign.search(tdCampaign.getTestData("SearchData", "TestData").adjust(TestData.makeKeyPath(
                SearchByField.class.getSimpleName(), SearchByField.CAMPAIGN_ID.getLabel()), childCampaignId));

        CampaignPage.tableCampaigns.getRow(1).getCell(CEMConstants.CEMCampaignsTable.CAMPAIGN_ID).verify.value(childCampaignId);
    }
}
