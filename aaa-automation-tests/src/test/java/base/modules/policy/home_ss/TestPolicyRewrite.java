 ...
 * @name Test Rewrite for Home Policy
 * @scenario
 * 1. Create Customer
...
public class TestPolicyRewrite extends HomeSSBaseTest {
    ...
    public void createCancelledPolicy() {
        ...
        effectiveDate = tdPolicy.getTestData("Rewrite", "TestDataNewNumber").getValue(
                HomeSSMetaData.RewriteActionTab.class.getSimpleName(),
                HomeSSMetaData.RewriteActionTab.EFFECTIVE_DATE.getLabel());
  ...
  /*  @BeforeMethod
    public void findPolicy() {
        if (!Page.comboBoxListAction.isPresent()) {
            mainApp().reopen();
