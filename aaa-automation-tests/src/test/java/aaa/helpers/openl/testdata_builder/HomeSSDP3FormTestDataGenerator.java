...
public class HomeSSDP3FormTestDataGenerator {
	...
	private static Function<HomeSSOpenLPolicy, List<TestData>> formDS0463DataFunction = openLPolicy -> {
		...
		for (HomeSSOpenLForm form : openLPolicy.getForms()) {
			if ("DS0463".equals(form.getFormCode())) {
				tdList.add(DataProviderFactory.dataOf(
						"Action", "Add",
						HomeSSMetaData.EndorsementTab.EndorsementDS0463.LOCATION_TYPE.getLabel(), "Residential".equals(form.getType()) ? "Described Location" : "Other Structure off Premises",
	...
	private static Function<HomeSSOpenLPolicy, List<TestData>> formDS0471DataFunction = openLPolicy -> {
		...
		tdList.add(DataProviderFactory.dataOf(
				"Action", "Add",
				HomeSSMetaData.EndorsementTab.EndorsementDS0471.COVERAGE_LIMIT.getLabel(), form.getCovPercentage() + "%"));
		return tdList;
	};
	...
	public enum Forms {
		...
		HS0495(HomeSSMetaData.EndorsementTab.DS_04_95.getLabel(), "HS0495", formDS0495DataFunction),    //temporary
