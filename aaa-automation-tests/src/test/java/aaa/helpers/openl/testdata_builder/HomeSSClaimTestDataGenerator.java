...
public class HomeSSClaimTestDataGenerator {
	...
	public List<TestData> getClaimTestData(boolean isAAAClaim, boolean isFirstClaim) {
		...
		sqlMaxCode = String.format("select max(code) from lookupvalue where lookuplist_id in (select id from lookuplist where lookupname='%s') and productcd='AAA_HO_SS' and (riskstatecd is null or riskstatecd = '%s')", lookupName, state);
		sqlMaxValue = String.format("select max(displayvalue) from lookupvalue where lookuplist_id in (select id from lookuplist where lookupname='%s') and productcd='AAA_HO_SS' and (riskstatecd is null or riskstatecd = '%s')", lookupName, state) + " and code=%d";
		sqlClaimData = String.format("select causeOfLoss, minPremiumOvr from lookupvalue where lookuplist_id in (select id from lookuplist where lookupname='%s') and productcd='AAA_HO_SS' and (riskstatecd is null or riskstatecd = '%s')", lookupName, state) + " and code=%d and displayvalue=%d order by riskstatecd";
	...
	private List<TestData> getClaimList(int claimPoints, int code) {
		...
		if (claimPoints < value) {
			...
			if (row.isEmpty()) {
				claimList.add(getClaim(DBService.get().getRow(String.format(sqlClaimData, code, 1))));
				if (--claimPoints > 0) {
