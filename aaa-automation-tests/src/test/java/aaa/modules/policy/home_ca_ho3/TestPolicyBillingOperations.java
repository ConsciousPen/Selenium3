...
public class TestPolicyBillingOperations extends HomeCaHO3BaseTest {
    ...
    public void testManualFeeAdjustment() {
        ...
        // 11. Check minimum due doesn't change
        BillingSummaryPage.tableBillingGeneralInformation.getRow(1).getCell(BillingConstants.BillingGeneralInformationTable.TOTAL_DUE).verify.contains(initialMinimumDue.toString());
    }
    ...
    public void testManualRefund() {
        ...
        refundActionTab.fillTab(tdSpecific.getTestData("TestData_Refund_1000").mask(
                TestData.makeKeyPath(BillingAccountMetaData.RefundActionTab.class.getSimpleName(), BillingAccountMetaData.RefundActionTab.AMOUNT.getLabel())));
        ...
        // 5. Check for an error message if Refund Amount > Total Paid Amount
        refundActionTab.fillTab(tdSpecific.getTestData("TestData_Refund_1000").adjust(
                TestData.makeKeyPath(BillingAccountMetaData.RefundActionTab.class.getSimpleName(), BillingAccountMetaData.RefundActionTab.AMOUNT.getLabel()), totalPaid.add(1).toString()));
        refundActionTab.submitTab();
        ...
        // 6. Check for an error message if Refund Amount is "0"
        refundActionTab.fillTab(tdSpecific.getTestData("TestData_Refund_1000").adjust(
                TestData.makeKeyPath(BillingAccountMetaData.RefundActionTab.class.getSimpleName(), BillingAccountMetaData.RefundActionTab.AMOUNT.getLabel()), "0"));
        refundActionTab.submitTab();
        refundActionTab.getAssetList().getWarning(BillingAccountMetaData.RefundActionTab.AMOUNT.getLabel()).verify.contains("Amount is required");
