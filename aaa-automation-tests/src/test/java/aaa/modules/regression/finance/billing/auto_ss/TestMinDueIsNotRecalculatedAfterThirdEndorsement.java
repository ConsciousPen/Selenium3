...
public class TestMinDueIsNotRecalculatedAfterThirdEndorsement extends FinanceOperations {
	...
	public void pas22575_testMinDueIsNotRecalculatedAfterThirdEndorsement(@Optional("KY") String state) {
		Dollar minDue;
		String policyNumber = openAppAndCreatePolicy(getPolicyTD());
		...
		minDue = new Dollar(BillingSummaryPage.tableBillingAccountPolicies
				.getRowContains(BillingConstants.BillingAccountPoliciesTable.POLICY_STATUS, ProductConstants.PolicyStatus.PROPOSED)
		...
		mainApp().open();
		SearchPage.openBilling(policyNumber);
		assertThat(new Dollar(BillingSummaryPage.tableBillingAccountPolicies
				.getRowContains(BillingConstants.BillingAccountPoliciesTable.POLICY_STATUS, ProductConstants.PolicyStatus.PROPOSED)
		...
		assertThat(BillingSummaryPage.tableBillsStatements.getValuesFromRows(BillingConstants.BillingBillsAndStatmentsTable.TYPE))
				.contains(BillingConstants.BillsAndStatementsType.DISCARDED_BILL);
		minDue = new Dollar(BillingSummaryPage.tableBillingAccountPolicies
				.getRowContains(BillingConstants.BillingAccountPoliciesTable.POLICY_STATUS, ProductConstants.PolicyStatus.PROPOSED)
				.getCell(BillingConstants.BillingAccountPoliciesTable.MIN_DUE).getValue());
	...
	private void endorseAndChangeBodilyInjury(String bodilyInjuryAmount) {
		...
		new PremiumAndCoveragesTab().getAssetList().getAsset(AutoSSMetaData.PremiumAndCoveragesTab.BODILY_INJURY_LIABILITY)
				.setValueContains(bodilyInjuryAmount);
