...
public abstract class PolicyBillingOperations extends PolicyBaseTest {
    ...
    public void testManualFeeAdjustment() {
        ...
        Dollar feeAmount =
                new Dollar(fee_transaction.getTestData(BillingAccountMetaData.OtherTransactionsActionTab.class.getSimpleName()).getValue(
    ...
    public void testManualRefund() {
        ...
        TestData payment =
                tdBilling.getTestData("AcceptPayment", "TestData_Check").adjust(
    ...
    public void testManualWriteOff() {
        ...
        Dollar writeoffAmount = new Dollar(writeoff.getTestData(BillingAccountMetaData.OtherTransactionsActionTab.class.getSimpleName())
    ...
    private void payment(TestData td, Dollar amount) {
        BillingSummaryPage.linkAcceptPayment.click();
        acceptPaymentActionTab.fillTab(td.adjust(
                TestData.makeKeyPath(BillingAccountMetaData.AcceptPaymentActionTab.class.getSimpleName(), BillingAccountMetaData.AcceptPaymentActionTab.AMOUNT.getLabel()), amount.toString()));
        AcceptPaymentActionTab.buttonOk.click();
    }
    ...
    private void declinePaymentAndVerify(Dollar amount, String reason) {
        ...
        BillingSummaryPage.tablePaymentsOtherTransactions.getRowContains(query).getCell(BillingPaymentsAndOtherTransactionsTable.ACTION).controls.links.get(PaymentsAndOtherTransactionAction.DECLINE)
                .click();
        if (!reason.isEmpty()) {
            new DeclinePaymentActionTab().getAssetList().getAsset(BillingAccountMetaData.DeclinePaymentActionTab.DECLINE_REASON.getLabel(), ComboBox.class).setValue(reason); //            new DeclineActionTab().getAssetList().getAsset(assetName).fillTab(new SimpleDataProvider().adjust(HomeCaMetaData.DeclineActionTab.class.getSimpleName(),
