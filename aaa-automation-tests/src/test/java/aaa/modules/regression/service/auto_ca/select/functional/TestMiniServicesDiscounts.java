...
public class TestMiniServicesDiscounts extends AutoCaSelectBaseTest {
	private static final String DISCOUNT_CODE_GOOD_STUDENT = "goodStudent";
	private static final String DISCOUNT_CODE_NEW_DRIVER = "newDriver";
	private static final String DISCOUNT_CODE_MATURE_DRIVER = "matureDriver";
	private final HelperMiniServices helperMiniServices = new HelperMiniServices();
	private static final TestMiniServicesDriversHelper DRIVERS_HELPER = new TestMiniServicesDriversHelper();
	...
	public void pas27988_miniServicesDiscountsTC01(@Optional("CA") String state) {
		mainApp().open();
	...
	/**
	 * @author Maris Strazds
	 * @name View Driver Service - CA Select and Driver Related Discounts - Good Student Discount, New Driver Discount, Mature Driver Discount
	 * @scenario
	 * 1. Create policy in PAS
	 * 2. Create Endorsement through service
	 * 3. Check that for drivers that already had these discounts applied at NB, they are not returned as Available (Good Student Discount, New Driver Discount, Mature Driver Discount)
	 */
	@Parameters({"state"})
	@Test(groups = {Groups.FUNCTIONAL, Groups.CRITICAL})
	@TestInfo(component = ComponentConstant.Sales.AUTO_CA_SELECT, testCaseId = {"PAS-27988"})
	public void pas27988_miniServicesDiscountsTC02(@Optional("CA") String state) {
		...
		TestData testData = td.
				...
				adjust(TestData.makeKeyPath(AutoCaMetaData.DocumentsAndBindTab.class.getSimpleName(),
						...
						AutoCaMetaData.DocumentsAndBindTab.RequiredToIssue.PROOF_OF_GOOD_STUDENT_DISCOUNT.getLabel()),
						"Yes").
				adjust(TestData.makeKeyPath(AutoCaMetaData.DocumentsAndBindTab.class.getSimpleName(),
						AutoCaMetaData.DocumentsAndBindTab.REQUIRED_TO_ISSUE.getLabel(),
	...
	private void driverAvailableDiscountsCheck(ViewDriversResponse viewDriversResponse, String discountCode, String discountName, String driverOid) {
		...
		assertThat(discount.oid).isNull();
	}
