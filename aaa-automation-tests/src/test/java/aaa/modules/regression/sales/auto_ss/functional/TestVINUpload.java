...
import static aaa.modules.regression.postconditions.TestVinUploadPostConditions.SELECT_VIN_NUMBER_BY_DATA;
...
public class TestVINUpload extends TestVinUploadHelper {
	...
	public void pas730_MSRPRefreshCompColl(@Optional("UT") String state) {
		...
		String vin = String.format(SELECT_VIN_NUMBER_BY_DATA,vehYear,vehMake,vehModel,vehSeries,vehBodyStyle);
		TestData testData = getPolicyTD()
				...
				.adjust(TestData.makeKeyPath(vehicleTab.getMetaKey(), AutoSSMetaData.VehicleTab.BODY_STYLE.getLabel()),vehBodyStyle);
		...
		Map<String, String> vinInfo = DBService.get().getRow(String.format(SELECT_VIN_NUMBER_BY_DATA,vehYear,vehMake,vehModel,vehSeries,vehBodyStyle));
	...
	public void testPolicyCreation(@Optional("SD") String state) {
		TestData testData = getTestDataWithMembershipSinceDate(getPolicyTD());
		TestData testdataPolicyInformation = testData.getTestData("GeneralTab").getTestData("PolicyInformation")
				.adjust("Effective Date","01/01/2018")
				.adjust("Override ASD Level","Yes")
				.adjust("Advance Shopping Discount Override","Level 1")
				.adjust("ASD Overriden By","QA");
		TestData testDataGeneralTab = testData.getTestData("GeneralTab")
				.adjust("PolicyInformation",testdataPolicyInformation);

		mainApp().open();
		createCustomerIndividual();
		createPolicy(testData.adjust("GeneralTab",testDataGeneralTab));
