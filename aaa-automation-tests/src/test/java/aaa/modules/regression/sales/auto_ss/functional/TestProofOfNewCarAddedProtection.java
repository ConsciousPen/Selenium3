...
public class TestProofOfNewCarAddedProtection extends AutoSSBaseTest {
    ...
    public void pas27186_testProofOfNewCarAddedProtectionEndorsement(@Optional("NV") String state) {

        String today = TimeSetterUtil.getInstance().getCurrentTime().format(DateTimeUtils.MM_DD_YYYY);
        TestData td = getPolicyTD().adjust(TestData.makeKeyPath(AutoSSMetaData.DocumentsAndBindTab.class.getSimpleName(), AutoSSMetaData.DocumentsAndBindTab.REQUIRED_TO_ISSUE.getLabel(),
                AutoSSMetaData.DocumentsAndBindTab.RequiredToIssue.PROOF_OF_PURCHASE_DATE_BILL_OF_SALE_FOR_NEW_VEHICLES.getLabel()), "Yes");
        validatePAS27186Endorsement(today, td);
        // Assert that 'Proof of purchase date (bill of sale) for new vehicle(s)' is set to yes
        assertThat(new DocumentsAndBindTab().getRequiredToIssueAssetList()
                .getAsset(AutoSSMetaData.DocumentsAndBindTab.RequiredToIssue.PROOF_OF_PURCHASE_DATE_BILL_OF_SALE_FOR_NEW_VEHICLES).getValue()).isEqualTo("Yes");
    }
    ...
    3. Add a 2019 vehicle.
    4. Navigate to Premium & Coverage tab.
    5. Add new car added protection with purchase date NB-35 days.
    6. Calculate premium.
    7. Navigate to Documents & Bind tab.
    8. Select 'YES' for 'Proof of equivalent new car added protection with prior carrier for new vehicle(s)'.
    9. Bind the quote.
    10. Initiate an endorsement on SS auto policy after 30 days of effective date.
    11. Navigate to Documents & Bind tab.
    12. Review 'Require to Issue' section.
    13. 'Proof of equivalent new car added protection with prior carrier for new vehicle(s)' still persists with Value to 'Yes'.
     *@details
     */
    @Parameters({"state"})
    @Test(groups = {Groups.FUNCTIONAL, Groups.HIGH})
    @TestInfo(component = ComponentConstant.Sales.AUTO_SS, testCaseId = "PAS-27186")
    public void pas27186_testProofOfNewCarAddedProtectionEndorsement1(@Optional("NV") String state) {

        String today = TimeSetterUtil.getInstance().getCurrentTime().minusDays(36).format(DateTimeUtils.MM_DD_YYYY);
