...
public class TestRemovingSppLowersTotalPremium extends HomeSSHO6BaseTest {
     ...
     * @name Test HO6 Removing SPP doesn't lower total premium
    ...
    @TestInfo(component = ComponentConstant.Sales.HOME_SS_HO6, testCaseId = "PAS-5847")
    public void pas5847_removingSPPDoesNotLowerTotalPremium(@Optional("NJ") String state) {
        mainApp().open();
        createCustomerIndividual();

        // adjust default testdata with testdata from ho3(re-usable)
        TestData testTd = getPolicyTD().adjust(testDataManager.getDefault(aaa.modules.regression.sales.home_ss.ho3.functional.TestRemovingSppLowersTotalPremium.class).resolveLinks());
